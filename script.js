...
function updateStats(gifts) {
    ...
    const highPriorityCount = gifts.filter(g => g.priority === 'high').length;
...
function createGiftElement(gift) {
    ...
    giftCard.innerHTML = `
        <div class="gift-header">
            <div class="gift-title">${gift.name}${purchasedText}</div>
            <div class="user-badge ${userClass}">${userLabel}</div>
        </div>
        ${gift.category ? `<div class="gift-category">${gift.category}</div>` : ''}
        ${gift.price ? `<div class="gift-price">${formatPrice(gift.price)} руб.</div>` : ''}
    ...
    // Для ссылки на товар
    const giftLinkElement = giftCard.querySelector('.gift-link');
    if (giftLinkElement) {
        giftLinkElement.addEventListener('click', (e) => {
            e.stopPropagation();
        });
    }
    ...
    modalBody.innerHTML = `
        <div class="modal-details">
            <div class="detail-row">
                ...
                <div class="detail-value">${gift.name}</div>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">${gift.category || 'Не указана'}</div>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">${gift.color || 'Не указано'}</div>
            </div>
            <div class="detail-row">
